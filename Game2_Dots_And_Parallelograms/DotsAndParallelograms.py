...
symbol_thickness = 50
#Changed hex codes for colors to make a milder color palette. Seth
dot_color = '#85ab78'
player1_color = '#525F84'
player1_color_light = '#7885AB'
player2_color = '#741F35'
player2_color_light = '#AB7885'
Green_color = '#85ab78'
dot_width = 0.25*size_of_board/number_of_dots
edge_width = 0.1*size_of_board/number_of_dots
distance_between_dots = 65 #changed distance to make altered board fit better. Sage
...
class Dots_and_Boxes():
    ...
    def mark_box(self):
        ...
        for box in boxes:
            if list(box) not in self.already_marked_boxes and list(box) !=[]:
                ...
                turn = 'A' #added to change from shaded box to letter marker. Sage
                self.shade_box(box, color, turn)
        ...
        for box in boxes:
            if list(box) not in self.already_marked_boxes and list(box) !=[]:
                ...
                turn = 'B' #added to change from shaded box to letter marker. Sage
                self.shade_box(box, color, turn)
    ...
    def make_edge(self, type, logical_position):
        '''
        determines start and end points of lines for boxes. 
        altered to make alternating parallelograms instead of squares
            added if else statements based on odd or even
        Alterations by Sage
        '''
        
        if type == 'row':
            if logical_position[1] % 2 == 0:
                ...
                start_x = logical_position[0]*distance_between_dots
                end_x = start_x+distance_between_dots
                start_y = distance_between_dots/2 + logical_position[1]*distance_between_dots
                end_y = start_y
            ...
            if logical_position[1] % 2 == 0:
                ...
                start_x = distance_between_dots / 2 + logical_position[0] * distance_between_dots
                end_x = start_x-(distance_between_dots/2)
            else:
                start_y = distance_between_dots / 2 + logical_position[1] * distance_between_dots
                end_y = start_y + distance_between_dots
                start_x = logical_position[0] * distance_between_dots
    ...
    def refresh_board(self):
        ...
        for i in range(number_of_dots):
            for j in range(number_of_dots):
                #draws dots
                #altered to make odd rows offset and centered, creating grid for parallelograms
                #altered by Sage
    ...
    def shade_box(self, box, color, turn):
        ...
        #create letter label for completed box instead of shading in the box. added by Sage
        mark = Label(text = turn, fg = color, font = ('Times New Roman', 25), padx = 3, pady = 3, bd = 0).place(x = start_x, y = start_y)
